...
const pageRows = ref(props.request.pageRows || props.writes.per_page.toString());
...
const updateSearch = () => {
    router.get(route('admin.board.write.list'), {
...
const isDeleteStatusUpdate = (writeId: number, operation: 'delete' | 'restore') => {
    router.put(route('admin.board.write.delete-status-update'), {
        writeId: writeId,
        operation: operation,
    }, {
        preserveState: true,
...
<template>
    <div class="px-4 pt-6 pb-3 block sm:flex items-center justify-between text-md">
        <div class="w-full">
            <Breadcrumb menu-name="게시판 관리" current-location="게시글 관리" />
                    ...
                    <table class="min-w-full table-fixed divide-y divide-gray-300 dark:divide-gray-600">
                        <thead class="bg-gray-100 dark:bg-gray-700 font-semibold">
                            <tr>
                                ...
                                <th scope="col" class="p-4 text-center text-gray-900 uppercase dark:text-white">조회</th>
                        ...
                        <tbody class="divide-y divide-gray-300 dark:divide-gray-700">
                            <tr v-for="(write, widx) in writes.data" :key="widx" class="bg-gray-100 dark:bg-gray-800 hover:bg-gray-300 dark:hover:bg-gray-700">
                                <td class="p-4 text-center text-gray-900 whitespace-nowrap dark:text-white">{{ write.board.table_name }}</td>
                                <td 
                                    class="p-4 text-left max-w-xl truncate whitespace-nowrap" 
                                    :class="{ 
                                        'text-gray-400 dark:text-gray-500 line-through': write.is_delete, 
                                        'text-gray-900 dark:text-white': !write.is_delete,
                                    }"
                                >
                                    <a :href="route('write.read', { tableId: write.board.table_id, writeId: write.id })" target="_blank">
                                        <span v-if="write.is_delete">[삭제됨]</span>{{ write.subject }}
                                    </a>
                                </td>
                                <td class="p-4 text-center text-gray-900 whitespace-nowrap dark:text-white">
                                    <span :class="{ 'text-green-500 dark:text-green-500 font-semibold': write.rate_up > 0 }">{{ formatNumberWithK(write.rate_up) }}</span> / 
                                    <span :class="{ 'text-red-500 dark:text-red-500 font-semibold': write.rate_down > 0 }">{{ formatNumberWithK(write.rate_down) }}</span>
                                </td>
                                <td class="p-4 text-center text-gray-900 whitespace-nowrap dark:text-white">{{ formatNumberWithK(write.comment_count) }}</td>
                                <td class="p-4 text-center text-gray-900 whitespace-nowrap dark:text-white">{{ formatNumberWithK(write.hit) }}</td>
                                <td class="p-4 text-center text-gray-900 whitespace-nowrap dark:text-white">
                                    <span v-if="write.report_count > 0">
                                        <ButtonColor 
                                            @click="openReportModal(write.id)"
                                            class="inline-flex w-full px-2 py-[2px] text-md font-medium items-center justify-center sm:w-auto" 
                                            :color="'yellow'"
                                        >
                                            {{ formatNumberWithK(write.report_count) }}
                                        </ButtonColor>
                                    </span>
                                    <span v-else>0</span>
                                </td>
                                <td class="p-4 text-center text-gray-900 whitespace-nowrap dark:text-white">{{ write.writer }}</td>
                                <td class="p-4 text-center text-gray-900 whitespace-nowrap dark:text-white">{{ dayjs(write.created_at).format('YYYY.MM.DD hh:mm') }}</td>
                                <td class="p-4 text-center text-gray-900 whitespace-nowrap dark:text-white">
                                    <ButtonColor 
                                        v-if="write.is_delete" 
                                        class="inline-flex w-full px-3 py-[5px] text-md font-medium items-center justify-center sm:w-auto" 
                                        :color="'teal'"
                                        @click="isDeleteStatusUpdate(write.id, 'restore')"
                                    >
                                    ...
                                    <ButtonColor 
                                        ...
                                        class="inline-flex w-full px-3 py-[5px] text-md font-medium items-center justify-center sm:w-auto" 
                                        :color="'red'"
                                        @click="isDeleteStatusUpdate(write.id, 'delete')"
                                    >
    ...
    <PaginationLinks :paginator="writes" />

    <Modal v-if="isShowModal" @handleModalClose="handleModalClose" :size="'md'" :position="'center'" :title="'신고 내역'">
        <BoardReportList @handleModalClose="handleModalClose" :targetTable="'writes'" :targetId="reportTargetId" />
    </Modal>
</template>
